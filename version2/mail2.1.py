...
def generate_document(title, link, abstract, count):
    ...
    <button style="font-size: 0.8em; background-color: red; color: white; margin-left: 4em;" onclick="toggleAbstract('abstract_{count}')"><b>摘要</b></button>
    <p id="abstract_{count}" style="display:none; font-size:1em; margin-top: 0.25em; margin-left: 4em;">{abstract}</p>
    <hr>
    '''
...
def generate_html_content(papers):
    ...
    for i, paper in enumerate(papers):
        ...
        if paper in included_docs:
            document = generate_document(title, link, abstract, focus_count)
            focus_content += document
            focus_count += 1
        elif paper in remaining_docs:
            document = generate_document(title, link, abstract, remaining_count)
            remaining_content += document
            remaining_count += 1
        else:
            document = generate_document(title, link, abstract, excluded_count)
