...
def generate_html_content(papers):
    ...
    if focus_content:
        output += f"<h2 style='color: rgb(139, 0, 18);'>Hot concerns:（标题关注）</h2>\n" + focus_content
    if focus_abstract_content:
        output += f"<h2 style='color: rgb(139, 0, 18);'>Guess you like it:（仅摘要关注）</h2>\n" + focus_abstract_content
    if remaining_content:
        output += f"<h2 style='color: rgb(139, 0, 18);'>Research trends:（无关键词）</h2>\n" + remaining_content
    if excluded_abstract_content:
        output += f"<h2 style='color: rgb(139, 0, 18);'>Read next time:（仅摘要排斥）</h2>\n" + excluded_abstract_content
    if excluded_content:
        output += f"<h2 style='color: rgb(139, 0, 18);'>Irrelevant topics:（标题排斥）</h2>\n" + excluded_content
...
# 另存为输出的HTML文件
def select_html_output(default_name):
    root = tk.Tk()
    root.withdraw()

    file_path = filedialog.asksaveasfilename(
        title="Save HTML Output As",
        initialfile=default_name,  # Set default filename
...
# 检查是否存在重复文档
def check_for_duplicates(papers, existing_files):
    existing_titles = set()
    for file_path in existing_files:
        existing_titles.update(read_file_content(file_path))

    # 仅保留不重复的文档
    unique_papers = [paper for paper in papers if paper['title'] not in existing_titles]
...
# 读取之前的文档标题用于查重
existing_titles = set()
temp_files = ['excluded_documents.txt', 'included_documents.txt', 
              'excluded_abstract_documents.txt', 'included_abstract_documents.txt', 
              'remaining_documents.txt']
for file_path in temp_files:
    existing_titles.update(read_file_content(file_path))

# 在更新文件之前进行查重
excluded_docs = check_for_duplicates(excluded_docs, existing_titles)
included_docs = check_for_duplicates(included_docs, existing_titles)
excluded_abstract_docs = check_for_duplicates(excluded_abstract_docs, existing_titles)
included_abstract_docs = check_for_duplicates(included_abstract_docs, existing_titles)
remaining_docs = check_for_duplicates(remaining_docs, existing_titles)
...
html_default_filename = base_filename + ".html"

# 调用函数并传递默认文件名
html_file_path = select_html_output(html_default_filename)
