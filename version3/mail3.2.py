...
def generate_document(title, link, abstract, count, section):
    ...
    for keyword in exclude_keywords:
        if re.search(r'\b' + re.escape(keyword.lower()) + r'\b', formatted_title.lower()):
            title_content = title_content.replace(keyword.lower(), f'<span style="font-size: 0.75em; color: green;"><b>{keyword}</b></span>')
            title_content = title_content.replace(keyword.upper(), f'<span style="font-size: 0.75em; color: green;"><b>{keyword}</b></span>')
            title_content = title_content.replace(keyword.capitalize(), f'<span style="font-size: 0.75em; color: green;"><b>{keyword}</b></span>')
    for keyword in include_keywords:
        if re.search(r'\b' + re.escape(keyword.lower()) + r'\b', formatted_title.lower()):
            title_content = title_content.replace(keyword.lower(), f'<span style="font-size: 1.25em; color: red;"><b>{keyword}</b></span>')
            title_content = title_content.replace(keyword.upper(), f'<span style="font-size: 1.25em; color: red;"><b>{keyword}</b></span>')
            title_content = title_content.replace(keyword.capitalize(), f'<span style="font-size: 1.25em; color: red;"><b>{keyword}</b></span>')
...
def filter_documents(document_list, exclude_keywords, include_keywords):
    ...
    for document in document_list:
        ...
        exclude_match = any(re.search(r'\b' + re.escape(keyword.lower()) + r'\b', title.lower()) for keyword in exclude_keywords)
        include_match = any(re.search(r'\b' + re.escape(keyword.lower()) + r'\b', title.lower()) for keyword in include_keywords)
